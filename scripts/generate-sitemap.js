    ...
    articles.forEach(article => {
        ...
        if (article.published_at) {
            try {
                ...
                // Reste sur la date du jour par défaut en cas d'erreur
            }
        }

        // Utilise le slug ou le génère à partir du titre
...
async function main() {
    try {
        ...
        console.log(`✅ Sitemap généré avec succès (${articles.length} URLs) à : ${SITEMAP_PATH}`);
        console.log(`📊 URLs générées : ${articles.length} articles`);
    } catch (error) {
